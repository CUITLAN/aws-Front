...
const CardInventory = async () => {
    ...
    return (
        <div style={{ display: 'flex', justifyContent: 'center', margin: 10, flexWrap: "wrap", gap: 10 }}>
            {inventory.length > 0 ? (
                inventory.map((item) => (
                    <Card className="p-4 m-2 bg-gray-300" key={item.id}>
                        <p className="text-default-500">ID: {item.id} </p>
                        <p className="text-default-500">Cantidad: {item.quantity} </p>
                        <p className="text-default-500">Cantidad Vendida: {item.sold_quantity} </p>
                        <p className="text-default-500">Ubicación: {item.bin_location_id} </p>
